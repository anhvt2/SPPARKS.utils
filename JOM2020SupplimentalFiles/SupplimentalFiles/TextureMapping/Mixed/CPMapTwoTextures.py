...
baseFileName = 'dump.additive4.1'
angleFileName1 = 'Fiber.csv'
angleFileName2 = 'Uniform.csv'
...
def grainSort(grainIDTable):
    ...
    for i in valuesThere.flatten():
        ...
        newGrainID[mask] = j
        if(volume < 25):
            eulerAngles[mask] = eulerIn2.iloc[j,:]
            #print("Setting layered texture " + str(meanZ))
        else:
            eulerAngles[mask] = eulerIn1.iloc[j,:]
        j = j + 1
...
#Lets also assign an orientation within the loop
#@jit
def centerOfMass(grainIDTable,i,axis):

    #If axis = 0, find average of z coordinates
    mask = grainIDTable.iloc[:,0] == i

    meanCoord = np.mean(grainIDTable[mask].iloc[:,4])
    #print(meanCoord)
...
#    smallMaskInverted = np.reshape(vectorMaskInverted,(nvx,ny,nz))
    ...
    rightShift[99,:,:] = 0 + segmented[99,:,:] * smallMask[99,:,:]
    ...
    leftShift[0,:,:] = 0 + segmented[0,:,:] * smallMask[0,:,:]
    ...
    frontShift[:,99,:] = 0 + segmented[:,99,:] * smallMask[:,99,:]
    ...
    backShift[:,0,:] = 0 + segmented[:,0,:] * smallMask[:,0,:]
    ...
    upShift[:,:,99] = 0 + segmented[:,:,99] * smallMask[:,:,99]
    ...
    downShift[:,:,0] = 0 + segmented[:,:,0] * smallMask[:,:,0]
...
def loadGrains():
	...
	latticeValues[reverseIndexHelper(range(totalSites))] = dataIn.iloc[:,0]
...
for i in range(firstFile, lastFile):
	...
	inputFile = os.path.join(cwd,str(i) + '/' + baseFileName)
	...
	eulerIn1 = pd.read_csv(angleFileName1, sep=',',header=None,index_col=None)
	eulerIn2 = pd.read_csv(angleFileName2, sep=',',header=None,index_col=None)
	...
	with open(fileName, mode = 'w') as f:
		...
		dataOut.to_csv(f,columns=(3,4,5,'phi1','phi2','Phi',1, 'ESD'),header=False,sep=' ', index=False)
