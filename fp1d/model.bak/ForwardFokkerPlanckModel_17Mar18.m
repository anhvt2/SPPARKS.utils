function statisticalDistance = ForwardFokkerPlanckModel(inputFokkerPlanck, pCheckAndMatch)
...
for i = 1:M
	...
	if (mod(i,Neverystep) == 0)
		currentClockTime = cputime();
		fprintf('Time-step %d/%d reached. Elasped time: %10.4f seconds; statisticalDistance = %10.4f.\n', i , M , (currentClockTime - startClockTime), checkStatisticalDistance);
	...
	if checkStatisticalDistance > 10;
		...
		fprintf('ForwardFokkerPlanckModel diverges.\n'); % notice
		return
	end
end
...
% pCheckAndMatch -- from SPPARKS model

statisticalDistance = 	L2norm(pSimCheck(1,:), pCheckAndMatch(11,:), x) + ... 
						L2norm(pSimCheck(2,:), pCheckAndMatch(20,:), x) + ... 
						L2norm(pSimCheck(3,:), pCheckAndMatch(29,:), x) + ... 
						L2norm(pSimCheck(4,:), pCheckAndMatch(38,:), x);

statisticalDistance = statisticalDistance * 100;
