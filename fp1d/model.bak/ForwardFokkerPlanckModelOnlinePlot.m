function statisticalDistance = ForwardFokkerPlanckModelOnlinePlot(inputFokkerPlanck)
...
N = 4.0e3; 								% number of segments
startSupp = -0.1; endSupp = 1.1;	 	% support
safeWidth = 1e-1;						% width of zero density
dt = 5e-3;								% time-step - fs
Nfrequency = 10/100 ;					% percentage - show progress at every Nfrequency

% computing time
startTime   = int32(80e3)/1e3;			% unit fs -> ps
endTime  	= int32(140e3)/1e3;			% unit fs -> ps
...
% inputFokkerPlanck = [5.13500783e-03 -1.27475697e-03 1.22675403e-06];
% inputFokkerPlanck = [5.12170045e-03,  -8.32483552e-04,   2.17014717e-06, 3.89975856e-08];
polyDrift = [inputFokkerPlanck(1), inputFokkerPlanck(2), inputFokkerPlanck(3)];
polyDiffusion = [inputFokkerPlanck(4), inputFokkerPlanck(5), inputFokkerPlanck(6)]; 
...
for i = 1:M
	...
	if (mod(i,Neverystep) == 0)
		...
		fprintf('Time-step %d/%d reached. Elasped time: %10.4f minutes.\n', i , M , (currentClockTime - startClockTime)/60);
	end
