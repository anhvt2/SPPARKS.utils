function statisticalDistance = ForwardFokkerPlanckModel(inputFokkerPlanck, pHistorySpparks)
...
N = 7.5e3; 								% number of segments
startSupp = 0.0e0; endSupp = 1.5e1;		% support
safeWidth = 1.0e0;						% width of zero density
dt = 1.0e-4;							% time-step
...
p0 = ksdensity(csvread(strcat(sourcePath,'/../probabilityRepo/','log.',var,'.15.dat')), x); % 21Feb22: modify for 'logAreaOfGrain' variable
pInit = p0;
...
for i = 1:M
	...
	if (mod(i,Neverystep) == 0)
		...
		% title(sprintf('Fokker-Planck density at iteration %d (time-step %6.2f). L2norm-distance = %6.4f' , ...
		% 				 i , i * dt + startTime, checkStatisticalDistance ) , 'HandleVisibility' , 'off' ) ;
		title(sprintf('Fokker-Planck density at iteration %d (time-step %6.2f).' , ...
...
if trapz(x,pSimCheck(4,:)) > 1.3 * trapz(x,pHistorySpparks(38,:));
	statisticalDistance = 1e+1;
	return
end

statisticalDistance = 	KLdistance(pHistorySpparks(38,:) , pSimCheck(4,:) , x) ;
