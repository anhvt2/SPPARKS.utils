...
inputFokkerPlanck
diary('log.Fokker-Planck.txt');
% ----------------------------------- PARAMETERS SETTINGS ---------------------------- %
var = 'logAreaOfGrain';					% name of variable
N = 1.0e4;								% number of segments
startSupp = 0.0e0; endSupp = 1.5e1;		% support
safeWidth = 5.0e-1;						% width of zero density
dt = 5.0e-4;							% time-step
% Nfrequency = 10/100 ;					% percentage - show progress at every Nfrequency -- deprecated
...
sourcePath = pwd;
Neverystep = 1.0e2;						% number of iteration to print images
h = (endSupp - startSupp)/N;
x = [startSupp:h:endSupp]; 				% discretized segment 


% ----------------------------------- INITIAL CONDITION ------------------------------- %
% checkTime = csvread(strcat(sourcePath,'/../probabilityRepo/','log.time.dat'));
checkTime = csvread(strcat(sourcePath,'/../probabilityRepo/','log.logtime.dat')); % 22Feb22
...
initData = csvread(strcat(sourcePath,'/../probabilityRepo/','log.',var,'.15.dat')); initData = initData(initData>1); % 21Feb22: filter the tail
p0 = ksdensity(initData, x); % 21Feb22: modify for 'logAreaOfGrain' variable
p0 = p0 / trapz(x,p0);
pInit = p0;

% pCalibrated = ksdensity(csvread(strcat(sourcePath,'/../probabilityRepo/','log.',var,'.32.dat')), x); % 17Mar18: modify to fit this particular example
% pCalibrated = ksdensity(csvread(strcat(sourcePath,'/../probabilityRepo/','log.',var,'.26.dat')), x); % 21Feb22: modify to fit this particular example
pCalibrated = ksdensity(csvread(strcat(sourcePath,'/../probabilityRepo/','log.',var,'.25.dat')), x); % 21Feb22: modify to fit this particular example
pCalibrated = pCalibrated / trapz(x,pCalibrated);
...
close all;
lambda = 1.000e-6; p0 = p0(:);
DMatrix = D2; % D2 and 1e-6 works surprisingly well for dt = 1e-3
...
% ----------------------------------- FINAL CONDITION ------------------------------- %
% pFinal = ksdensity(csvread(strcat(sourcePath,'/../probabilityRepo/','log.',var,'.39.dat')), x); % 17Mar18; modify to fit this particular example
% pFinal = ksdensity(csvread(strcat(sourcePath,'/../probabilityRepo/','log.',var,'.39.dat')), x); % 17Mar18; modify to fit this particular example
pFinal = ksdensity(csvread(strcat(sourcePath,'/../probabilityRepo/','log.',var,'.39.dat')), x); % 21Feb22
pFinal = pFinal / trapz(x,pFinal);
...
for i = 1:M
	...
	% pCurrent = pCurrent/trapz(x,pCurrent);		% normalize by areaOfGrain
	...
	% online plot -- turn on for debug
	if (mod(i,Neverystep) == 0) ||  ( i < 2500 && mod(i,100) == 0 ) || ( i < 100 && mod(i,5) == 0 ) || ismember(i, iCheck)
		currentClockTime = cputime();
		...
		% title(sprintf('Fokker-Planck density at iteration %d/%d (time-step %6.4f)' , i, M, double(i * dt + startTime)), 'HandleVisibility' , 'off' ) ;
		title(sprintf('Fokker-Planck density at iteration %d/%d (time-step %6.4f)' , i, M, double(i * dt + startTime)), 'HandleVisibility' , 'off' , 'FontSize', 24) ;

		% title(sprintf('Fokker-Planck density at iteration %d/%d (time-step %6.3f)' , i, M, currentTime), 'HandleVisibility' , 'off' ) ;
		...
		% graph(2) = plot(x(find(x>0)),pInit(find(x>0)) / trapz( x(find(x>0)), pInit(find(x>0)) ), 'ro:', 'LineWidth',2);
		% graph(4) = plot(x(find(x>0)),pCalibrated(find(x>0)) / trapz( x(find(x>0)), pCalibrated(find(x>0)) ), 'mo-.', 'LineWidth',2);
		% graph(3) = plot(x(find(x>0)),pFinal(find(x>0)) / trapz( x(find(x>0)), pFinal(find(x>0)) ), 'go--', 'LineWidth',2);
		...
		% legend('init density' , 'calibrated density', 'final density','current density');
		lgd = legend('first training pdf (kMC: 46.5 mcs)', 'last training pdf (kMC: 599.5 mcs)', 'last testing pdf (kMC: 16681.1 mcs)', 'evolving Fokker-Planck pdf');
		lgd.FontSize = 24;
		set(get(gca,'XLabel'),'String','support','FontSize',24,'FontWeight','bold','FontName','Times')
		set(get(gca,'YLabel'),'String','p.d.f','FontSize',24,'FontWeight','bold','FontName','Times')
		...
		xtickHandler = get(gca,'XTickLabel'); set(gca,'XTickLabel',xtickHandler,'FontName','Times','fontsize',24);
		ytickHandler = get(gca,'YTickLabel'); set(gca,'YTickLabel',ytickHandler,'FontName','Times','fontsize',24);

		dlmwrite(sprintf('pCurrent.%d.dat', i), pCurrent, 'delimiter', '\n'); % save pCurrent
	end
